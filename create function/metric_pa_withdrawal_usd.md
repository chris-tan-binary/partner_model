```
CREATE OR REPLACE FUNCTION development.update_partner_fact_pa_withdrawal_usd(
	p_start_date date,
	p_end_date date)
    RETURNS void
    LANGUAGE 'sql'
...
(select pp1.payment_ts::date as transaction_date,
	   ...
	   sum(pp1.transaction_amount) as withdrawal 
...
and lower(pp1.remark) like '%to payment agent%'
